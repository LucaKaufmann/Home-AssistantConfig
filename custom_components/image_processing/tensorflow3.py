...
class TensorFlowImageProcessor(ImageProcessingEntity):
    ...
    def process_image(self, image):
        ...
        image_tensor = self._graph.get_tensor_by_name('softmax:0')
        predictions = self._session.run(image_tensor,
            {'DecodeJpeg/contents:0': inp_expanded})
        ...
        for node_id in top_k:
            name = node_lookup.id_to_string(node_id)
