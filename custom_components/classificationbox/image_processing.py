...
PLATFORM_SCHEMA = PLATFORM_SCHEMA.extend({
    ...
    vol.Required(ATTR_MODEL_ID): cv.string,
...
def get_model(url, username, password):
    """Return the list of models."""
    kwargs = {}
    if username:
        kwargs['auth'] = requests.auth.HTTPBasicAuth(username, password)
...
def setup_platform(hass, config, add_devices, discovery_info=None):
    ...
    model_id = config.get(ATTR_MODEL_ID)
    url_health = "http://{}:{}/healthz".format(ip_address, port)
    hostname = check_box_health(url_health, username, password)
    if hostname is None:
        return

    url_model = 'http://{}:{}/{}/models/{}'.format(ip_address, port, CLASSIFIER, model_id)
    model = get_model(url_model, username, password)
    if model:
